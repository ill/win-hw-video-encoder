...
def generate_video_filename(video_filename, quality):
    """
    Generate output video file name in the format:
    """
    return f"{video_filename}-quality-{quality}.webm"

def generate_vmaf_filename(video_filename, quality):
    """
    Generate output VMAF file name in the format:
    "<video_filename>.vmaf.json"
    """
    return f"{video_filename}-quality-{quality}.vmaf.json"

def upload_to_s3(file):
    print(f"Uploading {file} to S3...")
    object_name = "videos/ffmpeg-video-quallity/sweep-quality/" + file
...
def main():
    ...
    csv_file = 'ffmpeg_vp9_sweep_quality.csv'
    csv_header = [
        'quality',
        #'link', 
        'video id',
    ...
    for quality in qualities:
        print(f"Running with \"difficult video\" quality: {quality}")

        output_filename = generate_video_filename("1440p-av1-42sec", quality)

        transcode_time = transcode("1440p-av1-42sec.mp4", quality, output_filename)

        vmaf_filename = generate_vmaf_filename("1440p-av1-42sec", quality)
        ...
        # Construct S3 link
        s3_link = f"https://audiovisual-test-public.s3.us-east-1.amazonaws.com/videos/ffmpeg-video-quallity/sweep-quality/{output_filename}"
