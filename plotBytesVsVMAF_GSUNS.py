...
# Refactor this some time so there's shared code, for now this is just a modification for the GSUN csvs

@dataclass
class DimensionSettings:
    enable_1pass: bool = True
    enable_2pass: bool = True
    gsun_range: Optional[Tuple[float, float]] = None  # (min_gsun, max_gsun) or None
...
def plot_combined_bd_class_dim(
    ...
    for file_idx, file_settings in enumerate(file_settings_list):
        ...
        df['gsun'] = pd.to_numeric(df['gsun'], errors='coerce')
        ...
        for (width, height), dsettings in dims_settings.items():
            ...
            if dsettings.gsun_range is not None:
                min_gsun, max_gsun = dsettings.gsun_range
                sub = sub[(sub['gsun'] >= min_gsun) & (sub['gsun'] <= max_gsun)]
            if sub.empty:
                continue
            ...
            for pass_type in pass_types:
                ...
                plot_data.append({
                    ...
                    'gsun': group['gsun'].values[sort_idx],
    ...
    if use_leader_lines:
        ...
        for i, pdict in enumerate(plot_data):
            ...
            label_points = []
            label_colors = []
            for idx, (xi, yi, nxi, nyi, gsun, file_bytes) in enumerate(zip(x, y, nx, ny, pdict['gsun'], pdict['file_bytes'])):
                label_parts = []
                if show_gsun:
                    label_parts.append(f"{int(gsun)}")
                if show_filesize:
        ...
        for i, pdict in enumerate(plot_data):
            ...
            for x, y, gsun, file_bytes in zip(pdict['file_MB'], pdict['vmaf_mean'], pdict['gsun'], pdict['file_bytes']):
                label_parts = []
                if show_gsun:
                    label_parts.append(f"{float(gsun)}")
                if show_filesize:
                    label_parts.append(f"({format_bytes(file_bytes)})")
                label_str = " ".join(label_parts)
                txt = ax.text(
...
# Example usage:
# Plot all widths:
# plot_bd('your_file.csv', min_gsun=18, max_gsun=42)
# Plot a specific width:
# plot_bd('your_file.csv', width=1920, min_gsun=18, max_gsun=42)

#plot_bd('Out/CRF/sonichd/sonichd-CRF.csv', min_gsun=18, max_gsun=42)
...
def highGraphs():
    ...
    file_structs = [
        FileSettings(
            ...
            dims={
                (1920, -1): DimensionSettings(enable_1pass=False),
                (1280, -1): DimensionSettings(enable_1pass=False),
                (640, -1): DimensionSettings(enable_1pass=False)
            },
            label='SonicHD'
        ),
        FileSettings(
    ...
    plot_combined_bd_class_dim(
        ...
        output_filename='combined_bd_graph_gsun.png',
        show_plot=True,
...
def lowMotionGraphs():
    ...
    plot_combined_bd_class_dim(
        file_settings_list=file_structs,
        output_filename='combined_low_motion_bd_graph_gsun.png',
        show_plot=True,
        use_leader_lines=False,
        show_gsun=True,
        show_filesize=False
