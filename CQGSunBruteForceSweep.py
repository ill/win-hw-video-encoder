...
class CQGSunBruteForceSweep(CQExperiment.CQExperiment):
    ...
    def get_extra_csv_header_columns(self):
        return ['gsun'] + super().get_extra_csv_header_columns()
    ...
    def run_experiment_on_resolution(self, resolution: tuple[int, int]):
        ...
        min_bitrate_target_pct_min = 0
        min_bitrate_target_pct_max = 100
        min_bitrate_target_pct_step = 10

        max_bitrate_target_pct_min = 100
        max_bitrate_target_pct_max = 200
        max_bitrate_target_pct_step = 10
        ...
        for crf in range(crf_min, crf_max + crf_step, crf_step):
            for fps in framerates:
                for gsun in gsuns:
                    ...
                    for min_bitrate_target_pct in range(min_bitrate_target_pct_min, min_bitrate_target_pct_max, min_bitrate_target_pct_step):
                        for max_bitrate_target_pct in range(max_bitrate_target_pct_min, max_bitrate_target_pct_max, max_bitrate_target_pct_step):
                            for threads in range(threads_min, threads_max + threads_step, threads_step):
                                for tile_columns in range(tile_columns_min, tile_columns_max + tile_columns_step,
                                    ...
                                    cq_params.min_bitrate_kbps = int(target_bitrate * (min_bitrate_target_pct / 100))
                                    cq_params.max_bitrate_kbps = int(target_bitrate * (max_bitrate_target_pct / 100))
                                    ...
                                    for speed_single_pass in range(speed_single_pass_min,
                                        ...
                                        self.run_experiment_on_cq_params_one_pass(resolution, fps, gsun, cq_params=cq_params)
                                    ...
                                    for speed_pass_1 in range(speed_pass_1_min, speed_pass_1_max + speed_pass_1_step,
                                        ...
                                        for speed_pass_2 in range(speed_pass_2_min, speed_pass_2_max + speed_pass_2_step,
                                            ...
                                            self.run_experiment_on_cq_params_two_pass(resolution, fps, gsun, cq_params=cq_params)

    def run_experiment_on_cq_params_one_pass(self, resolution: tuple[int, int], fps: int, gsun: float,
                                             cq_params: CQExperiment.CQExperiment.SubExperiment.CQParams):
    ...
    def run_experiment_on_cq_params_two_pass(self, resolution: tuple[int, int], fps: int, gsun: float,
                                             cq_params: CQExperiment.CQExperiment.SubExperiment.CQParams):
    ...
    class SubExperiment(CQExperiment.CQExperiment.SubExperiment):
        ...
        def get_extra_csv_columns(self):
            return [str(self.gsun)] + super().get_extra_csv_columns()