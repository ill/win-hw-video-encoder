    ...
    def run_experiment_on_resolution(self, resolution: tuple[int, int]):
        ...
        for crf in range(crf_min, crf_max + crf_step, crf_step):
            for target_bitrate in range(target_bitrate_min, target_bitrate_max + target_bitrate_step,
                                        target_bitrate_step):
                for min_bitrate in range(min(min_bitrate_kbps_min, target_bitrate),
                                         min(min_bitrate_kbps_max, target_bitrate) + min_bitrate_kbps_step,
                                         min_bitrate_kbps_step):
                    for max_bitrate in range(max(max_bitrate_kbps_min, target_bitrate),
                                             max(max_bitrate_kbps_max, target_bitrate) + max_bitrate_kbps_step,
                                             max_bitrate_kbps_step):
