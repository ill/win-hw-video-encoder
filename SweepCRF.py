...
def generate_video_filename(video_filename, crf):
    """
    Generate output video file name in the format:
    """
    return f"{video_filename}-crf-{crf}.webm"

def generate_vmaf_filename(video_filename, crf):
    """
    Generate output VMAF file name in the format:
    "<video_filename>.vmaf.json"
    """
    return f"{video_filename}-crf-{crf}.vmaf.json"

def upload_to_s3(file):
    print(f"Uploading {file} to S3...")
    object_name = "videos/ffmpeg-video-quallity/sweep-crf/" + file
...
def transcode(input_filename, crf, output_filename):
...
def main():
    ...
    csv_file = 'ffmpeg_vp9_sweep_crf.csv'
    csv_header = [
        'crf',
        #'link', 
        'video id',
    ...
    for crf in range(0, 63, 5):
        print(f"Running with \"difficult video\" crf: {crf}")

        output_filename = generate_video_filename("1440p-av1-42sec", crf)

        transcode_time = transcode("1440p-av1-42sec.mp4", crf, output_filename)

        vmaf_filename = generate_vmaf_filename("1440p-av1-42sec", crf)
        ...
        # Construct S3 link
        s3_link = f"https://audiovisual-test-public.s3.us-east-1.amazonaws.com/videos/ffmpeg-video-quallity/sweep-crf/{output_filename}"
